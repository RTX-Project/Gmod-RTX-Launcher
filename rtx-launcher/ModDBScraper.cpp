...
const wchar_t* kModDBScraperScript = 
    ...
    L"    if (el) nav(el);"
    L"    else {"
    L"      var links = document.querySelectorAll('a');"
    L"      var found = false;"
    L"      var modDownloadsUrl = url.split('?')[0];"
    ...
    L"          window.chrome.webview.postMessage('NAVIGATE_TO:' + mirrorUrl);"
    L"        } else {"
    L"          log('Failed to find mirror in fetch!');"
    L"        }"
    L"      }).catch(function(e) { log('Fetch error: ' + e); });"
    L"    } else {"
    L"      log('Could not find file download button on ' + url);"
    L"    }"
                        ...
                        [state](HRESULT result, ICoreWebView2Controller* controller) -> HRESULT {
                            ...
                            state->webView->add_NavigationCompleted(Callback<ICoreWebView2NavigationCompletedEventHandler>(
                                [state](ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT {
                                    ...
                                    std::wstring script = kModDBScraperScript;
